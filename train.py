...
from StackedHourglass import StackedHourglassForKP, StackedHourglassForDesc, StackedHourglassImgRecon
...
from utils import make_transformation_M, my_dataset
...
def train():
    ...
    model_StackedHourglassImgRecon = StackedHourglassImgRecon(nstack=num_nstack, inp_dim=stacked_hourglass_inpdim_kp, oup_dim=3, bn=False, increase=0)
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            combined_hm_preds = model_StackedHourglassForKP(aefe_input.cuda())
            ...
            fn_DetectionConfidenceMap2keypoint = fn_DetectionConfidenceMap2keypoint.cuda()
            DetectionMap, keypoints, zeta = fn_DetectionConfidenceMap2keypoint(combined_hm_preds.cuda(), batch_size, num_of_kp)
            ...
            relu4descriptors = torch.nn.ReLU().cuda()
            get_descriptors = relu4descriptors(get_descriptors).cuda()
            #print("get_descriptors", get_descriptors.shape) #(b,k,96,128)

            #tensor_1 = get_descriptors
            #tensor_2 = DetectionMap
            Wk_raw = get_descriptors.cuda() * DetectionMap.cuda()
            ...
            fn_softmask = create_softmask().cuda()
            ...
            my_descriptor = []
            for b in range(softmask.shape[0]):
                for k in range(softmask.shape[1]):
                    mul = 0
                    for i in range(softmask.shape[2]):
                        for j in range(softmask.shape[3]):
                            mul_1 = softmask[b, k, i, j]
                            mul_2 = get_descriptors[b, k, i, j]
                            mul += mul_1 * mul_2
                    mul_all_torch[b, k, :] = mul

            my_descriptor = (mul_all_torch * Wk_).cuda() #(b, 16, 128)
            print("my_descriptor_gen time: ", time.time() - descriptor_gen_time)

            recon_start = time.time()
            fn_reconDetectionMap = ReconDetectionConfidenceMap().cuda()
            reconDetectionMap = fn_reconDetectionMap(keypoints, reconDetectionMap_std_x, reconDetectionMap_std_y, correlation, DetectionMap)
            ...
            loss = 1 * cur_conc_loss.cuda() + 0.5 * cur_sep_loss.cuda() + 250*cur_recon_loss.cuda()
            print("Loss=>", "concentration loss: ", cur_conc_loss.item(), ",", "separation loss: ", cur_sep_loss.item(), "," , "cur_recon_loss: ", cur_recon_loss.item())
            #vis.line(Y=[torch.tensor(loss)], X=[torch.tensor(my_iteration)], win=plot, update='append')
